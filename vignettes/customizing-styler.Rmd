...
how? Its input only is a flat parse table. Since the visitor functions `pre_visit()`
...
the opening bracket and setting the column `spaces` of the token to one. Note
that it is very important to check whether there is also a line break
following after that token. If so, `space` should not be touched because of
...
that, indention before the token after `(` would be removed, which we don't
want.
...
withough specifying them inside the function passed to `transformers`.
...
described above. These columns are to some degree redundant obviously (execpt
...
* modify the line breaks (modifying `lag_newlines` only).
* modify the spaces (modifying `spaces` only based on `lag_newlines`, 
  `newlines` and `multi_line` and `token` and `text`).
...
* modify the indention by changing `indention_ref_id` (based on `newlines` 
  `lag_newlines`, `spaces` `multi_line`, `token` and `text`).
...
will be set after line breaks are set. Do not rely on the information `col1`,
`col2`, `line1` and `line2` in any of your function since these columns do only
reflect the position of tokens at the point of parsing, i.e. they are not kept
up to date through the process of styling.
